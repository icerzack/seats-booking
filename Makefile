...
k8s-clean:
	...
	-kubectl --kubeconfig=$(KUBECONFIG_PATH) delete -f k8s/ingress-frontend.yaml 2>/dev/null || echo "Frontend Ingress not found"
	-kubectl --kubeconfig=$(KUBECONFIG_PATH) delete -f k8s/ingress-backend.yaml 2>/dev/null || echo "Backend Ingress not found"
	-kubectl --kubeconfig=$(KUBECONFIG_PATH) delete -f k8s/ingress-bot.yaml 2>/dev/null || echo "Bot Ingress not found"
	@echo "Deleting HPA..."
...
k8s-deploy-full:
	...
	@KUBECONFIG=$(KUBECONFIG_PATH) kubectl apply -f k8s/ingress-frontend.yaml
	@KUBECONFIG=$(KUBECONFIG_PATH) kubectl apply -f k8s/ingress-backend.yaml
	@KUBECONFIG=$(KUBECONFIG_PATH) kubectl apply -f k8s/ingress-bot.yaml
...
help:
	...
	@echo "  make sonar-help                   - Show SonarQube commands"
	@echo "  make sonar-start                  - Start SonarQube locally"
	@echo "  make sonar-test                   - Run tests with coverage"
	@echo "  make sonar-stop                   - Stop SonarQube"
	@echo ""
	@echo "🚀 CI/CD Integration:"
	@echo "  make ci-setup-secrets             - Guide for setting up GitHub secrets"
	@echo "  make ci-test-kubeconfig           - Test KUBECONFIG locally"
	@echo "  make ci-simulate-pipeline         - Simulate full CI/CD pipeline"
	@echo ""
	@echo "🔧 Usage Examples:"
...
.PHONY: sonar-test sonar-backend sonar-frontend sonar-start sonar-stop sonar-install sonar-help

sonar-help:
	@echo "🔍 SonarQube commands:"
	@echo "  make sonar-install    - Install SonarQube locally"
	@echo "  make sonar-start      - Start SonarQube containers"
	@echo "  make sonar-stop       - Stop SonarQube containers"
	@echo "  make sonar-backend    - Run backend tests with coverage"
	@echo "  make sonar-frontend   - Run frontend tests with coverage"
	@echo "  make sonar-test       - Run all tests with coverage"

sonar-install:
	@echo "🚀 Installing SonarQube locally..."
	@chmod +x sonarqube/install-sonarqube.sh
	@echo "📝 Note: This will install SonarQube in Docker"
	@echo "🌐 After installation, access at: http://localhost:9000"
...
sonar-test: sonar-backend sonar-frontend
	...
	@echo ""
	@echo "🔍 Next steps:"
	@echo "   1. Start SonarQube: make sonar-start"
	@echo "   2. Open http://localhost:9000"
	@echo "   3. Create projects and run analysis"

# CI/CD INTEGRATION COMMANDS
.PHONY: ci-setup-secrets ci-test-kubeconfig ci-simulate-pipeline
...
ci-setup-secrets:
	...
	@echo ""
	@echo "1️⃣  KUBECONFIG"
	@echo "   Description: Base64-encoded kubeconfig file"
	@echo "   How to get it:"
	@echo "   ┌─────────────────────────────────────────────────────────────┐"
	@echo "   │ cat ~/.kube/config | base64 -w 0                           │"
	@echo "   └─────────────────────────────────────────────────────────────┘"
	@echo ""
	@echo "2️⃣  SONAR_HOST_URL (optional, for SonarQube integration)"
	@echo "   Example: http://your-sonarqube-server:9000"
	@echo ""
	@echo "3️⃣  SONAR_TOKEN (optional, for SonarQube integration)"
	@echo "   Get from: SonarQube → My Account → Security → Generate Token"
	@echo ""
	@echo "📍 Where to add secrets:"
	@echo "   GitHub Repository → Settings → Secrets and variables → Actions"
	@echo ""
	@echo "🚀 Next step: Test your kubeconfig locally"
	@echo "   make ci-test-kubeconfig KUBECONFIG_PATH=/path/to/your/kubeconfig.yaml"

ci-test-kubeconfig:
	@echo "🧪 Testing KUBECONFIG locally..."
	@if [ -z "$(KUBECONFIG_PATH)" ]; then \
		echo "❌ Error: KUBECONFIG_PATH must be specified"; \
		echo "Example: make ci-test-kubeconfig KUBECONFIG_PATH=/path/to/kubeconfig.yaml"; \
		exit 1; \
	fi
	@if [ ! -f "$(KUBECONFIG_PATH)" ]; then \
		echo "❌ Kubeconfig file not found: $(KUBECONFIG_PATH)"; \
	...
	@echo "📁 Testing kubeconfig file: $(KUBECONFIG_PATH)"
	@echo ""
	@echo "🔍 Testing cluster connection..."
	@KUBECONFIG=$(KUBECONFIG_PATH) kubectl cluster-info || (echo "❌ Cluster connection failed" && exit 1)
	@echo ""
	@echo "🗃️  Testing base64 encoding (for GitHub secrets)..."
	@BASE64_CONFIG=$$(cat $(KUBECONFIG_PATH) | base64 -w 0 2>/dev/null || cat $(KUBECONFIG_PATH) | base64); \
	echo "✅ Kubeconfig successfully encoded to base64 (length: $$(echo $$BASE64_CONFIG | wc -c) chars)"
	@echo ""
	@echo "🎯 Testing deployment commands..."
	@echo "Testing namespace access..."
	@KUBECONFIG=$(KUBECONFIG_PATH) kubectl get namespaces | head -5
	@echo ""
	@echo "✅ KUBECONFIG is valid and ready for CI/CD!"
	@echo ""
	@echo "📝 Copy this base64 value to GitHub Secrets as KUBECONFIG:"
	@echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
	@cat $(KUBECONFIG_PATH) | base64 -w 0 2>/dev/null || cat $(KUBECONFIG_PATH) | base64
	@echo ""
	@echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

ci-simulate-pipeline:
	@echo "🎭 Simulating CI/CD Pipeline locally..."
	@if [ -z "$(KUBECONFIG_PATH)" ]; then \
		echo "❌ Error: KUBECONFIG_PATH must be specified"; \
		echo "Example: make ci-simulate-pipeline KUBECONFIG_PATH=/path/to/kubeconfig.yaml"; \
		exit 1; \
	fi
	@echo ""
	@echo "🏗️  Stage 1: Build & Test"
	@echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
	@echo "📦 Building backend..."
	@cd backend && mvn clean compile -q
	@echo "📦 Building frontend..."
	@cd frontend && npm install --silent > /dev/null 2>&1 && npm run build > /dev/null
	@echo "✅ Build completed"
	@echo ""
	@echo "🧪 Stage 2: Testing & Coverage"
	@echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
	@echo "🔍 Running backend tests..."
	@cd backend && mvn test jacoco:report -q
	@echo "🔍 Running frontend tests..."
	@cd frontend && npm test -- --coverage --watchAll=false --silent > /dev/null 2>&1 || echo "Frontend tests completed"
	@echo "✅ Tests completed"
	@echo ""
	@echo "🔍 Stage 3: SonarQube Analysis (Simulated)"
	@echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
	...
	@echo "✅ Quality analysis completed (simulated)"
	@echo ""
	@echo "🚀 Stage 4: Kubernetes Deployment"
	@echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
	@echo "☸️  Deploying to Kubernetes..."
	@make k8s-deploy-full KUBECONFIG_PATH=$(KUBECONFIG_PATH)
	@echo ""
	@echo "🎉 Pipeline simulation completed!"
	@echo ""
	@echo "📊 Final status:"
	@make k8s-status KUBECONFIG_PATH=$(KUBECONFIG_PATH)
