...
class BookingServiceTest {
    ...
    void givenBookingCreateDTO_whenCreateBooking_thenReturnUserCodeDTO() {
        ...
        doNothing().when(bookingServiceSpy).sendToTelegramController(any(Booking.class));

        UserCodeDTO result = bookingServiceSpy.createBooking(bookingCreateDTO);

        assertNotNull(result);
        assertEquals(userCode.getCode(), result.getCode());
        verify(bookingRepository, times(1)).save(any(Booking.class));
        verify(bookingServiceSpy, times(1)).sendToTelegramController(any(Booking.class));
    }
